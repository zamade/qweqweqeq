if (game:GetService("CoreGui")):FindFirstChild("ZATHEON") and (game:GetService("CoreGui")):FindFirstChild("ScreenGui") then
	(game:GetService("CoreGui")).ZATHEON:Destroy();
...
(getgenv()).LoadConfig = function()
	if readfile and writefile and isfile and isfolder then
		if not isfolder("ZATHEON") then
			makefolder("ZATHEON");
		end;
		if not isfolder("ZATHEON/Library/") then
			makefolder("ZATHEON/Library/");
		end;
		if not isfile(("ZATHEON/Library/" .. game.Players.LocalPlayer.Name .. ".json")) then
			writefile("ZATHEON/Library/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(SettingsLib));
		else
			local Decode = (game:GetService("HttpService")):JSONDecode(readfile("ZATHEON/Library/" .. game.Players.LocalPlayer.Name .. ".json"));
...
(getgenv()).SaveConfig = function()
	if readfile and writefile and isfile and isfolder then
		if not isfile(("ZATHEON/Library/" .. game.Players.LocalPlayer.Name .. ".json")) then
			...
			writefile("ZATHEON/Library/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(Array));
		end;
	else
		return warn("Status : Undetected Executor");
	end;
...
function Update:Window(Config)
	...
	local ZATHEON = Instance.new("ScreenGui");
	ZATHEON.Name = "ZATHEON";
	ZATHEON.Parent = game.CoreGui;
	ZATHEON.DisplayOrder = 999;
	local OutlineMain = Instance.new("Frame");
	OutlineMain.Name = "OutlineMain";
	OutlineMain.Parent = ZATHEON;
...
Grad3.Parent = Sep3;
	...
	function osfunc:CreatePlayerCard()
		...
		-- Main Card Container positioned at bottom of sidebar
		local CardContainer = Instance.new("Frame");
		CardContainer.Name = "PlayerCard";
		CardContainer.Parent = self.SidebarTab;
		CardContainer.BackgroundColor3 = Color3.fromRGB(20, 10, 10);
		CardContainer.BackgroundTransparency = 0;
		CardContainer.Size = UDim2.new(1, -10, 0, 70);
		CardContainer.Position = UDim2.new(0, 5, 1, -70);
		...
		CardCorner.Parent = CardContainer;

		-- Card Border
		local CardStroke = Instance.new("UIStroke");
		CardStroke.Color = Color3.fromRGB(200, 30, 30);
		CardStroke.Thickness = 2;
		CardStroke.Parent = CardContainer;
		...
		AvatarFrame.Parent = CardContainer;
		...
		-- Avatar Stroke (Border)
		local AvatarStroke = Instance.new("UIStroke");
		AvatarStroke.Color = Color3.fromRGB(200, 30, 30);
		...
		InfoFrame.Parent = CardContainer;
		InfoFrame.BackgroundTransparency = 1;
		InfoFrame.Size = UDim2.new(1, -70, 1, 0);
		InfoFrame.Position = UDim2.new(0, 66, 0, 0);
		...
		UserIDLabel.Text = "ID: 0";

		-- Tag Label
		local TagLabel = Instance.new("TextLabel");
		TagLabel.Name = "Tag";
		TagLabel.Parent = InfoFrame;
		TagLabel.BackgroundColor3 = Color3.fromRGB(200, 30, 30);
		...
		TagLabel.Size = UDim2.new(0, 60, 0, 16);
		TagLabel.BorderSizePixel = 0;
		TagLabel.Text = "Lifetime";
		...
		-- Return functions to customize the card
		local cardFunctions = {};
		function cardFunctions:SetTag(newTag)
			TagLabel.Text = newTag;
		end;
		function cardFunctions:SetTagColor(bgColor, textColor)
			TagLabel.BackgroundColor3 = bgColor;
			TagLabel.TextColor3 = textColor;
		end;
		function cardFunctions:SetUsername(name)
			UsernameLabel.Text = name;
		end;
		function cardFunctions:SetUserID(id)
